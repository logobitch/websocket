    ...
    function run()
    {
        while (true) {
            ...
            foreach ($changes as $sign) {
                if ($sign == $this->master) {
                    //接收一个新的用户并建立连接
                    error_log('ok! now do accpet a new user');
                    ...
                    error_log('return user info back!');
                    ...
                    $k = $this->search($sign);
                    $user = $this->users[$k];
                    //error_log('event log length is'. print_r($len, 1));
                    if ($len < 7) {
                        $this->close($sign);
                        $eventreturn = array('k' => $k, 'sign' => $sign);
                    ...
                    if (!$user['hand']) {//没有握手进行握手
                        error_log('do one handshake');
                        $this->handshake($k, $buffer);
                    } else {
                        error_log('do send one message');
